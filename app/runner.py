...
from .comun import (
    ...
    REINTENTOS,
    RESOLUCION_UVT,
    TEXTO_DECLARACIONES_PRESENTADAS,
    TEXTO_DESCARGAR_RECIBO,
    TEXTO_FORMULARIO_RENTA,
    TEXTO_GENERAR_RECIBO,
    TEXTO_RUT_FAVORITO,
    TEXTO_SI_YES,
    TEXTO_SIN_DATOS_FE,
    TIMEOUT_DESCARGA,
...
class DianRunner:
    ...
    async def descargar_recibo_pago(
        ...
        Flujo MUISCA verificado:
          Diligenciar y presentar -> Formulario 210 -> "Declaraciones de renta
          presentadas" -> Pagar (fila del año) -> SI/YES -> fecha de pago ->
          Generar recibo -> Descargar recibo de pago (PDF).

        El recibo resultante corresponde al formulario 490, NUNCA al 210.
        Errores legibles:
          - No hay declaración presentada para el año pedido.
          - La declaración no tiene valor a pagar (botón Pagar inactivo).
          - La fecha de pago es anterior al mínimo permitido por la DIAN.
        """
        if anio not in ANIOS_RECIBO_RENTA:
            raise ValueError(
                "El año de la declaración debe ser uno de: "
        ...
        if not re.fullmatch(r"\d{4}-\d{2}-\d{2}", fecha_pago):
            raise ValueError("La fecha de pago debe tener formato AAAA-MM-DD.")
        ...
        self.loguear(
            f"[info] Recibo de pago: {tipo_documento} {numero_documento} | "
            f"renta {anio} | fecha pago {fecha_pago}"
        )
        self.emitir_done(
            f"Ingresando al portal DIAN ({num_documento_mascarado(numero_documento)})..."
        )
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(
                    "No se encontró la opción 'Declaraciones de renta presentadas'. "
                    f"({type(exc).__name__})"
                ) from exc
            self.loguear("  [recibo] Declaraciones de renta presentadas")
            await page.wait_for_timeout(5000)

            # 3) Botón Pagar de la fila del año solicitado.
            etiqueta_fila = f"{anio} / {ETIQUETA_AÑO_ANUAL}"
            try:
                fila = page.locator("mat-row", has_text=etiqueta_fila).first
                await fila.wait_for(timeout=15000)
                boton_pagar = fila.locator(
                    "button.buttonIconAccion img[matTooltip='Pagar']"
                ).first
                await boton_pagar.wait_for(timeout=8000)
                await boton_pagar.locator("xpath=ancestor::button[1]").click(
                    force=True
                )
            except Exception as exc:
                raise RuntimeError(
                    f"No se encontró una declaración de renta presentada para el "
                    f"año {anio}, o la fila no expone el botón Pagar. "
                    f"({type(exc).__name__})"
                ) from exc
            self.loguear(f"  [recibo] Pagar (fila {anio})")

            # 4) Modal de confirmación SI/YES.
            await page.wait_for_timeout(4000)
            try:
                await page.get_by_role(
                    "button", name=TEXTO_SI_YES, exact=True
                ).click(timeout=10000)
            except Exception as exc:
                raise RuntimeError(
                    "El pago no está habilitado para esa declaración "
                    "(la declaración no tiene valor a pagar o su plazo no es "
                    "aprovechable). " + f"({type(exc).__name__})"
                ) from exc
            self.loguear("  [recibo] Confirmación SI/YES")
            await page.wait_for_timeout(6000)

            # 5) Fecha de pago en el formulario "Datos de pago".
            try:
                campo_fecha = page.locator("input[type='date']").first
                await campo_fecha.fill(fecha_pago)
            except Exception as exc:
                raise RuntimeError(
                    "No se encontró el campo de fecha de pago. "
                    f"({type(exc).__name__})"
                ) from exc

            # 6) Generar recibo y luego descargarlo (PDF 490).
            try:
                await page.get_by_role(
                    "button", name=TEXTO_GENERAR_RECIBO, exact=True
                ).click(force=True, timeout=10000)
            except Exception as exc:
                raise RuntimeError(
                    "No se encontró el botón 'Generar recibo'. "
                    f"({type(exc).__name__})"
                ) from exc
            self.loguear("  [recibo] Recibo generado, descargando PDF...")
            await page.wait_for_timeout(5000)

            destino = (
                self.download_dir
                / f"ReciboRenta_{anio}_{numero_documento}_{fecha_pago}.pdf"
            )
            for intento in range(1, REINTENTOS + 1):
                try:
                    btn_descarga = page.get_by_role(
                        "button", name=TEXTO_DESCARGAR_RECIBO, exact=True
                    )
                    await btn_descarga.wait_for(timeout=20000)
                    async with page.expect_download(
                        timeout=TIMEOUT_DESCARGA
                    ) as dl_info:
                        await btn_descarga.click(force=True)
                        download = await dl_info.value
                    nombre_real = download.suggested_filename or ""
                    await download.save_as(destino)
                    if destino.exists() and destino.stat().st_size > 0:
                        self.loguear(
                            f"  [ok] Recibo descargado: {destino} "
                            f"({nombre_real})"
                        )
                        return destino
                    raise RuntimeError("la descarga quedó vacía")
                    ...
                    self.loguear(
                        f"  [recibo][reintento {intento}/{REINTENTOS}] "
                        f"{type(exc).__name__}: {exc}"
                    )
                    if intento == REINTENTOS:
                        raise
                    await page.wait_for_timeout(1500)
            raise RuntimeError("No se pudo descargar el recibo de pago.")
        finally:
            await context.close()
            await browser.close()
            await p.stop()
